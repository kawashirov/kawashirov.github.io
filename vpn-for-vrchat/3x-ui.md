...
- зайти во вкладку **Routing Rules** и настроить правила под warp как угодно, проще всего:
  ...
  - теперь нужно указать что именно мы хотим роутить в warp,
  для этого проще всего выставить какое-то поле, которое покроет весь трафиик,
  например `Source IPs` = `0.0.0.0/0` или `Network` = `TCP,UDP` -- это **нужно**,
  т.к. **из-за пустых правил XRay выдаёт ошибку**.
