...
![amneziawg-warp-transfer-received.png](./amneziawg-warp-transfer-received.png)
...
Вариантов не много:
1. [Использовать более **новую версию AmneziaWG**: 1.5 или лучше **2**.](./amneziawg-v2.md)
2. Сгенерировать **новый конфиг с другими опциями**.
Генерировать новый с теми же опциями вряд ли поможет.
  - Обратите внимание: некоторые генераторы предлагают конфиги **для AmneziaWG 1.5 или 2**, они не будут работать на версиях 1.0.x.
3. Поменять Peer **Endpoint** на другой.
...
4. Поменять **настройки обфускации AmneziaWG**.
Внутри конфига есть опции-циферки, их можно немного менять, чтобы добиться работоспособности.
Например, у меня (пока) работает:
```ini
Jc = 4
Jmin = 40
Jmax = 70
...
А для нормальной работы стриминга с ПК в VR-шлем, программам вроде **Virtual Desktop**, **ALVR**, **Steam Link**, **Oculus Link** и т.п. нужен свободный доступ к локальной сети.

Чтобы убрать **kill-switch** нужно просто изменить настройки туннеля (кнопка **Edit**):
- поменяв значение строки `AllowedIPs` в разделе `[Peer]`
    - с `0.0.0.0/0, ::/0`
    - на **`0.0.0.0/1, 128.0.0.0/1, ::/1, 8000::/1`**

- или просто **убрать** галочку "**Block untunneled traffic (kill-switch)**" внизу.
