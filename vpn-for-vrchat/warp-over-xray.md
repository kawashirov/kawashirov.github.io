...
Не буду расписывать подробно, т.к. метод довольно экзотический. В общих чертах:
...
Т.е. получается цепочка: клиент <--> "Домашний" XRay <--> XRay на VPS.
...
См. пример ниже.
Т.е. я как будто бы подключаюсь к компу в ловальной сети по обычному WireGuard, а не к Cloudflare WARP, но XRay роутит трафик на Cloudflare WARP сам.
