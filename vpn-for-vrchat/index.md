...
logo: /vpn-for-vrchat/opengraph-image-480x270.png
---
...
- [AmneziaWG 1.5 и 2](./amneziawg-v2.md)
- [Домены и IP VRChat](./domains-and-ips.md)


# Ситуация на данный момент
Формально, VRChat **не** заблокирован в России.
...
А уже эти сервисы регулярно блокируются в России по тем или иным причинам. 
...
Подразумевается, что пользователи должны **заходить в VRChat только из своих домашних сетей**.
На эту политику VRChat уже давно жалуются не только в России, но и западных сообществах.
...
- **не** блокируются, либо блокируются с большим трудом: **AmneziaWG** (желательно 1.5, 2 или выше), **VLESS** (Reality), **Hysteria2** -- на эти протоколы следует смотреть **в первую очередь**.
...
### Spamhaus
У Spamhaus, списками которого VRChat и пользуется для блокировок VPN по IP со своей стороны,
есть сервис **проверки адресов** на "вшивость" -- [**check.spamhaus.org**](https://check.spamhaus.org/).
Забейте туда **IP вашего VPN/VPS**, чтобы удостовериться.
(Узнать IP можно на сайтах по типу [**ipinfo.io/what-is-my-ip**](https://ipinfo.io/what-is-my-ip).)
Даже если сейчас бана нет, можно увидеть как давно он был в прошлом.
Если IP вашего VPN/VPS фигурирует в этом списке, то в VRChat вас не пустит с практически 100% шансом.
Если IP блокировался часто/недавно, то вас может снова начать не пускать в ближайшем будущем.
Когда VRChat блокирует таким образом, то он выдаёт ошибки вида:

`{"error":{"message":""the IP address or network you're trying to connect from is blacklisted (spamhaus_asndrop list). if you're using a vpn or proxy, try disabling it first. if you believe to have gotten this message in error, please contact support at help.vrchat.com"","status_code":403,"waf_code":42860,"waf_list":"spamhaus_asndrop"}}`

(Забавно, кстати, что это не валидный JSON, лол.)

С **полными DROP-списками Spamhaus** можно ознакомиться здесь:
...
### Также
- [Тут можно найти список VPS хостингов, принимающих **оплату криптой**](https://bitcoin-vps.com/#VPS-providers). 
...
Если перечисленное на этих страницах (VPN/zapret/WARP) **решило** ваши проблемы с соединением в VRChat,
а **YouTube работает** должным образом **в браузере**, но **не работает в** самом **VRChat**,
то, скорее всего, **пророблема в другом**.

Здесь я рассматриваю проблему **на примере** YouTube,
но вообще это относится в том числе и **к другим сервисам видео и музыки**.
...
**VRChat проворачивает трюк**, используя "под капотом" **отдельную программу** - [**`yt-dlp`**](https://github.com/yt-dlp/yt-dlp?tab=readme-ov-file#readme).
...
### И что же делать?
Решение есть: [**VRCVideoCacher**](https://github.com/EllyVR/VRCVideoCacher).
...
Однако, оно **не решает проблему блокировки YouTube и других сервисов в России**,
по этому вам **всё ещё необходимы VPN/zapret/WARP/прочее**,
а также **необходимо соблюдать все** описанные на этих страницах **тонкости**.
