...
- Во-первых, все средства борьбы против DPI очень спицифичны. В зависимости от **времени, места, провайдера** и т.п. DPI на **ТСПУ работает по-разному** и такой обход может как работать, так и **не** работать. Это **не самый надёжный** и **не самый универсальный** метод на данный момент.
...
- В-третих, иногда античиту EAC **не** нравится, когда zapret работает как служба (service)... а иногда ему всё равно, не ясно от чего это зависит. Если просто запускать zapret, то обычно проблем с античитом нет.
- В-четвёртых, конфиги zapret обычно фокусируются на HTTPS (TCP) трафиике, а VRChat также использует UDP на адресах, которые иногда попадают под блокировки. Ещё, некоторые кривые конфиги пытаясь заблокировать QUIC блокируют UDP целиком.
...
Ещё, в случае VRChat, нужно смотреть, что бы VPN/VPS **не были в чёрных списках VRChat**.
Заранее узнать это точно - невозможно, по этому тут только советоваться с людьми, которые **уже используют VPN в VRChat**.
...
По этому, вам следует использовать "постоянные" VPN, т.е. те, которые всегда обеспечивают вам **одну локацию** и **IP из одной и той же подсети**.
...
Ещё важно, что бы всё было **проксифицировано одинаково**,
...
если забыть зщавернуть траффик сервисов Epic Games,
то античит EAC может начать сбоить.
По этому лучше заворачивать **процессы** VRChat целиком и вместе.
...
напрмер через в v2rayN или Nekobox в режиме "Do not change system proxy" и "Enable Tun = Off".
